...
<template>
	<div class="material-type">
		<div class="item" v-for="item in materialTypeList" :key="item.id">
			<div class="top">
				<h3>{{ item.name }}</h3>
				<Button type="text" size="small" @click="emit('selectType', item.id)">View more</Button>
			</div>
			<div class="img-box">
...
const getMaterialTypesHandler = async () => {
	...
	materialTypeList.value = res.data.data
		.filter((el) => lng.map((it) => it.id).indexOf(el.id) >= 0)
		.map((item) => {
			return {
				// name: item.attributes.name,
				name: lng.find((el) => el.id === item.id)?.name ?? item.attributes.name,
				id: item.id,
				list: []
			}
		})
